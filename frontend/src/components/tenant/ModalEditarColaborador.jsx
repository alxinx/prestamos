...
import { IcoMas, IcoCorreo, IcoX } from './iconos'
...
export default function ModalEditarColaborador({ colaborador, onCerrar, onActualizado, onDesactivado }) {
  ...
  function alternarPermiso(codigo) {
    setPermisos(p => p.map(perm => (perm.codigo === codigo ? { ...perm, activo: !perm.activo } : perm)))
  }
  ...
  return createPortal(
    ...
    >
      <div className="w-[70vw] max-w-[70vw] max-sm:w-[94vw] max-sm:max-w-[94vw] max-h-[85vh] bg-surface-lowest rounded-2xl shadow-card-hover flex flex-col overflow-hidden animate-[slideUp_0.25s_cubic-bezier(0.34,1.56,0.64,1)]">

        {/* Header */}
        <div className="px-6 py-4 border-b border-outline-variant/50 flex items-center justify-between shrink-0">
          <div>
            <h2 className="text-[16px] font-bold text-on-background m-0">Editar datos y permisos</h2>
            <p className="text-[12px] text-on-surface-variant m-0">{colaborador.nombreCompleto}</p>
          </div>
          <button
            onClick={onCerrar}
            aria-label="Cerrar"
        ...
        {/* Body */}
        <div className="flex-1 overflow-y-auto grid grid-cols-1 sm:grid-cols-2 gap-6 p-6">
